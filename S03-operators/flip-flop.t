...
# make sure {lhs,rhs} isn't evaluated when state is {true,false}
{
    ...
    sub ff_eval($code, $lhs, $rhs, @a) {
        ...
        my $lhs_code = {$lhs_run++; $_ ~~ $lhs};
        my $rhs_code = {$rhs_run++; $_ ~~ $rhs};

        for @a {
            if $code.($_, $lhs_code, $rhs_code) {
            }
        }
    ...
    is_deeply ff_eval({$_ = @_[0]; @_[1] ff @_[2]}, /B/, /B/, <A B A B A>),
        [5, 2], "count lhs & rhs evals for ff";

    is_deeply ff_eval({$_ = @_[0]; @_[1] fff @_[2]}, /B/, /B/, <A B A B A>),
